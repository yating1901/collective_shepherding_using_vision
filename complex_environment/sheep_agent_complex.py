...
class Sheep_Agent(pygame.sprite.Sprite):
    ...
    def __init__(self, id, radius, position, orientation, env_size, color,
                 window_pad, target_x, target_y, target_size, is_antagonistic,
                 alpha, list_of_obstacles):
        """
        ...
        self.f_obstacle_x = 0.0
        self.f_obstacle_y = 0.0
        self.f_obstacle_r = 0.0
        self.is_obstacle = False
        self.K_obstacle = 3500
    ...
    def avoid_obstacles(self):
        ...
        for obstacle in self.list_of_obstacles:
            save_distance = obstacle["radius"] + self.radius*4
            d_sheep_obstacle = np.sqrt((self.x - obstacle["x"]) ** 2 + (self.y - obstacle["y"]) ** 2)
            if d_sheep_obstacle < save_distance and d_sheep_obstacle > obstacle["radius"]:
                #num_obstacle = num_obstacle + 1
                f_obstacle_r = 1 / abs(save_distance - d_sheep_obstacle)
                self.is_obstacle = True
                self.state = "collision"
            elif d_sheep_obstacle < obstacle["radius"]:
                f_obstacle_r = 1 / abs(obstacle["radius"] - d_sheep_obstacle)
                self.is_obstacle = True
                self.state = "collision"

            f_obstacle_x = ((self.x - obstacle["x"]) / (d_sheep_obstacle + 0.000001))  # *((self.safe_distance - distance )**2)
            f_obstacle_y = ((self.y - obstacle["y"]) / (d_sheep_obstacle + 0.000001))  # *((self.safe_distance - distance )**2)
        #if num_shepherd != 0:
        self.f_obstacle_x = f_obstacle_r * f_obstacle_x
        self.f_obstacle_y = f_obstacle_r * f_obstacle_y
        self.f_obstacle_r = f_obstacle_r
    ...
    def update(self, agents, shepherd_agents):  # this is actually sheep_agents;
        ...
        if self.num_rep != 0:
            ...
            self.f_x = self.f_obstacle_x * self.K_obstacle #3500
            self.f_y = self.f_obstacle_y * self.K_obstacle #3500
        else:
